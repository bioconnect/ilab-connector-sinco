...
namespace clawSoft.clawPDF.Workflow
{
    ...
    internal class InteractiveWorkflow : ConversionWorkflow
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine("kskang(clawPDF:InteractiveWorkflow) QueryTargetFile (7)");

            // 파일저장 다이얼로그 및 저장후 뷰어 패스 하도록 설정 (kskang)
            ...
            {
                // SDMS connector 실행 여부 확인(kskang)
                Process[] pro = Process.GetProcessesByName("SDMS Connector");
                if (pro.Length <= 0)
                {
                    MessageBox.Show("SDMS Connector를 먼저 실행해 주시기 바랍니다.");
                    Cancel = true;
                    WorkflowStep = WorkflowStep.AbortedByUser;
                    return;
                }
                else
                {
                    ...
                    // 여기에 파일패스 담으면 자동 저장됨. (kskang)
                    // 원본 파일 정보 가져오기
                    System.Diagnostics.Trace.WriteLine(String.Format("kskang(clawPDF:InteractiveWorkflow) Job.JobInfo.SourceFiles[0].Filename = {0}", Job.JobInfo.SourceFiles[0].Filename));
                    System.Diagnostics.Trace.WriteLine(String.Format("kskang(clawPDF:InteractiveWorkflow) Job.JobInfo.SourceFiles[0].DocumentTitle = {0}", Job.JobInfo.SourceFiles[0].DocumentTitle));
                    string fileNameWithoutExtension = Path.GetFileNameWithoutExtension(Job.JobInfo.SourceFiles[0].DocumentTitle);

                    string pdfFileFullPath = Environment.GetFolderPath(System.Environment.SpecialFolder.MyDocuments) + @"\SDMSConnector\ManualTemp\" + fileNameWithoutExtension + ".pdf";
                    Job.OutputFilenameTemplate = pdfFileFullPath;
                    System.Diagnostics.Trace.WriteLine(String.Format("kskang(clawPDF:InteractiveWorkflow) QueryTargetFile OutputFilenameTemplate = {0}", Job.OutputFilenameTemplate));
            ...
            else
            {
                ...
                System.Diagnostics.Trace.WriteLine("kskang(clawPDF:InteractiveWorkflow) QueryTargetFile FileName0 = ", saveFileDialog.FileName);
                ...
                System.Diagnostics.Trace.WriteLine("kskang(clawPDF:InteractiveWorkflow) QueryTargetFile FileName1 = ", saveFileDialog.FileName);
        ...
        private bool LaunchSaveFileDialog(SaveFileDialog saveFileDialog)
        {
            ...
            System.Diagnostics.Trace.WriteLine("kskang(clawPDF:InteractiveWorkflow) LaunchSaveFileDialog saveFileDialog.FileName0 = ", saveFileDialog.FileName);
            ...
            {
                // kskang test
                //var outputFile = @"C:\Users\inglo\Downloads\clawpdftest\kskang.pdf";
                ...
                System.Diagnostics.Trace.WriteLine("kskang(clawPDF:InteractiveWorkflow) LaunchSaveFileDialog outputFile = ", outputFile);
